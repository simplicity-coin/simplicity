...
- [Development guidelines](#development-guidelines)
    - [General Simplicity](#general-simplicity-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Simplicity uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play SPLs (tSPL)" on the test network, if you
...
Simplicity is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/simplicity-project/simplicity/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_simplicity
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_simplicity --log_level=test_suite
$ valgrind -v --leak-check=full src/simplicityd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_simplicity.coverage/index.html`.
```
...
- BitcoinMiner : Generates SPLs (if wallet is enabled).
...
However, in open source software such as Simplicity, where everyone uses
...
pay attention to for reviewers of Simplicity code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Simplicity uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Simplicity so that they can be integrated
...
file descriptors it will cause problems with Simplicity's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Simplicity-Project/Simplicity.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `simplicity-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `simplicity-cli` and the GUI debug console use this table to determine how to
