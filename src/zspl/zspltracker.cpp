...
CzSPLTracker::CzSPLTracker(std::string strWalletFile)
{
...
CzSPLTracker::~CzSPLTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSPLTracker::Init()
{
...
bool CzSPLTracker::Archive(CMintMeta& meta)
{
...
bool CzSPLTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSPLTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSPLTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSPLTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSPLTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSPLTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSPLTracker::GetSerialHashes()
{
...
CAmount CzSPLTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSPLTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSPLTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSPLTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSPLTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSPLTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSPLTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSPLTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSPLTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSPLTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSPLTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSPLWallet* zSPLWallet)
{
    bool iszSPLWalletInitialized = (NULL != zSPLWallet);
    CMintMeta meta;
    ...
    if (! iszSPLWalletInitialized)
        zSPLWallet = new CzSPLWallet(strWalletFile);
    meta.isSeedCorrect = zSPLWallet->CheckSeed(dMint);
    if (! iszSPLWalletInitialized)
        delete zSPLWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSPLTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSPLTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSPLTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSPLTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSPLTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSPLTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSPLWallet* zSPLWallet = new CzSPLWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSPLWallet);
        }
        delete zSPLWallet;
        LogPrint("zero", "%s: added %d dzspl from DB\n", __func__, listDeterministicDB.size());
    }
