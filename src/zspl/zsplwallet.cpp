...
CzSPLWallet::CzSPLWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zspl seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSPLSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSPLSeed_deprecated()) {
                    LogPrintf("%s: Updated zSPL seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zspl seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zspl wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSPLWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSPLCount(nCountLastUsed);
    else if (!walletdb.ReadZSPLCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSPLWallet::Lock()
{
    seedMaster = 0;
}

void CzSPLWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSPLWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSPL(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSPLWallet::LoadMintPoolFromDB()
{
...
void CzSPLWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSPLWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSPLWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zsplTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSPLWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSPL(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zsplTracker which also adds to database
    pwalletMain->zsplTracker->Add(dMint, true);
...
void CzSPLWallet::SeedToZSPL(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSPLWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSPLWallet::GenerateDeterministicZSPL(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSPLWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSPLWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSPLWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
