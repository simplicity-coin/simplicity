...
#include "ui_zsplcontroldialog.h"
...
std::set<std::string> ZSplControlDialog::setSelectedMints;
std::set<CMintMeta> ZSplControlDialog::setMints;

bool CZSplControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZSplControlDialog::COLUMN_DENOMINATION || column == ZSplControlDialog::COLUMN_VERSION || column == ZSplControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZSplControlDialog::ZSplControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSplControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSplControlDialog::~ZSplControlDialog()
{
    delete ui;
}

void ZSplControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSplControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZSplControlWidgetItem* itemDenom(new CZSplControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZSplControlWidgetItem *itemMint = new CZSplControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zsplTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zsplTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSPL.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSPL seed used to mint this zSPL is not the same as currently hold in the wallet";
            else
...
void ZSplControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSplControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSpl_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSplControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZSplControlDialog::GetSelectedMints()
{
...
void ZSplControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
